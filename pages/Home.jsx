...
const HomePage = () => {
  ...
  // ✅ Export Engineering Duty Schedule as PDF (All Shifts in Tabular Form)
  const exportDutyScheduleToPDF = () => {
    try {
      const doc = new jsPDF();
      ...
      doc.setFontSize(18);
      doc.setTextColor(40);
      doc.text(`Engineering Duty Schedule`, pageWidth / 2, 20, { align: 'center' });
      doc.setFontSize(12);
      doc.text(`Date: ${selectedDate}`, pageWidth / 2, 26, { align: 'center' });

      let startY = 32;

      const tableColumn = ['Shift', 'Engineers', 'Technician', 'Electrician'];
      const tableRows = [];

      shifts.forEach((shiftTime) => {
        const duty = filteredDuties.find((d) => d.shiftTime === shiftTime);
        if (!duty) return;

        const engineerNames = duty.engineers.map(e => `${e.name} (${e.status})`).join('\n');
        const technician = `${duty.technician.name} (${duty.technician.status})`;
        const electrician = `${duty.electrician.name} (${duty.electrician.status})`;
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          ...
          1: { cellWidth: 60 },     // Engineers
          2: { cellWidth: 40 },     // Technician
          3: { cellWidth: 40 },     // Electrician
        }
      });
